...
#
# After all 6 sentinels, an iql_v6_v2_summary.json is compiled.
...
OUT=runs/iql_v6_v2_a1_vs_joint_${STAMP}
LOG_DIR=logs/iql_v6_v2_a1_vs_joint_${STAMP}
mkdir -p ${OUT} ${LOG_DIR}
echo "[iql-v6_v2] OUT=${OUT}" | tee ${OUT}/launcher.log
echo "[iql-v6_v2] LOG_DIR=${LOG_DIR}" | tee -a ${OUT}/launcher.log
...
run_lane() {
    ...
    echo "[iql-v6_v2] launched lane g${GPU} ${LANE_NAME} pid=$(cat ${LANE_OUT}/.lane.pid)" | tee -a ${OUT}/launcher.log
}
...
EXPECTED=6
echo "[iql-v6_v2] $(date +%Y-%m-%d_%H:%M:%S) all ${EXPECTED} lanes launched; watching for sentinels..." | tee -a ${OUT}/launcher.log
while true; do
    DONE=$(find ${OUT}/ -maxdepth 2 -name .lane_done 2>/dev/null | wc -l)
    if [ "${DONE}" -ge "${EXPECTED}" ]; then
        echo "[iql-v6_v2] $(date +%Y-%m-%d_%H:%M:%S) all ${EXPECTED} lanes done" | tee -a ${OUT}/launcher.log
        break
    fi
    sleep 60
done
...
(out / "iql_v6_v2_summary.json").write_text(json.dumps(summary, indent=2))
print("=== IQL v6_v2 A1 vs JOINT SUMMARY ===")
print(json.dumps(summary, indent=2))
PYEOF

echo "[iql-v6_v2] $(date +%Y-%m-%d_%H:%M:%S) CHAIN DONE" | tee -a ${OUT}/launcher.log
